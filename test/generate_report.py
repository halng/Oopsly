...
class TestReportGenerator:
    """Generates Markdown, HTML and JSON test reports."""
    ...
    def generate_markdown_report(self, test_results: Dict[str, Any]) -> Path:
        """Generate Markdown report."""
        report_name = f"test_report_{self.release_version}_{self.timestamp}.md"
        report_path = self.output_dir / report_name
        
        markdown_content = self._build_markdown(test_results)
    ...
    def _build_markdown(self, test_results: Dict[str, Any]) -> str:
        ...
        markdown = f"""# Test Report - Release {self.release_version}
...
| **⏭️ Skipped** | {skipped} | {(skipped/total_tests*100 if total_tests > 0 else 0):.1f}% |

### {pass_emoji} Pass Rate: {pass_rate:.1f}%

"""
        
        # Add status indicator
        if pass_rate >= 90:
            markdown += "✨ **Excellent!** All tests passing or near-perfect pass rate.\n\n"
        elif pass_rate >= 70:
            markdown += "⚠️ **Warning:** Some tests are failing. Review required.\n\n"
        else:
            markdown += "🚨 **Critical:** High failure rate. Immediate attention needed!\n\n"
        
        markdown += "---\n\n## 📋 Test Details\n\n"
        
        # Add test details by suite
            ...
            for suite_name, suite_data in suites.items():
                ...
                if tests:
                    ...
                    for test in tests:
                        ...
                        markdown += f"| `{test['name']}` | {status_icon} {status} | {test.get('duration', '0')}s |\n"
                    
                    markdown += "\n"
        else:
            markdown += "_No detailed test data available._\n\n"
        
        markdown += "---\n\n"
        markdown += "## 📈 Statistics\n\n"
        markdown += f"- **Test Execution Time:** {test_results.get('total_duration', 'N/A')}\n"
        markdown += f"- **Environment:** {test_results.get('environment', 'default')}\n"
        markdown += f"- **Release Version:** `{self.release_version}`\n"
        markdown += f"- **Timestamp:** `{self.timestamp}`\n\n"
        
        markdown += "---\n\n"
        markdown += "_Generated by Oopsly Test Suite_\n"
...
def main():
    ...
    # Generate reports (Markdown is primary format)
    markdown_report = generator.generate_markdown_report(test_results)
