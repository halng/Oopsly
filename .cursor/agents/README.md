...
2.  **The Action:**
    *   The **Architect** decides if this needs a new Go microservice or can be added to an existing Java/Spring Boot service. They draft an ADR and update the C4 model.
...
2.  **The Action:** The Solution Designer acts as the translator. They design the exact PostgreSQL tables or MongoDB collections. They write the OpenAPI/Swagger JSON specifications that will dictate exactly how the React-Native frontend will talk to the backend.
...
2.  **The Action:**
    *   The **Engineer** writes the actual production code. Because the contracts are already defined, they don't have to guess how the API should look. They just implement the logic in Java/Go or build the UI in React-Native.
...
2.  **The Action:**
    *   The **Code Reviewer** scans the code for idiomatic standards (Google Java Style, Effective Go), security flaws, and performance bottlenecks. You iterate with the Engineer until the Reviewer approves.
