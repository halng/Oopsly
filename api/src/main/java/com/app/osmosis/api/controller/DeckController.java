...
public class DeckController {
    ...
    public ApiRes createDeck(
            @Valid @RequestBody CreateDeck createDeck,
            @AuthenticationPrincipal UserDetails userDetails) {
        UUID userId = UUID.fromString(userDetails.getUsername());
        return deckService.createDeck(createDeck, userId);
    }
    ...
    public ApiRes updateDeck(
            ...
            @Valid @RequestBody UpdateDeck updateDeck,
            @AuthenticationPrincipal UserDetails userDetails) {
        UUID userId = UUID.fromString(userDetails.getUsername());
        return deckService.updateDeck(id, updateDeck, userId);
    }

    @PatchMapping("/{id}")
    public ApiRes softDeleteDeck(
            @PathVariable UUID id, @AuthenticationPrincipal UserDetails userDetails) {
        UUID userId = UUID.fromString(userDetails.getUsername());
        return deckService.softDeleteDeck(id, userId);
    }
}
