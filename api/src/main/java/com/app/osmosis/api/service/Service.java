 ...
 * @param <K> the type of the entity's identifier such as Long, String, etc.
 * @param <S> the type of the request object used for creating/updating the entity
 */
public interface Service<T, K, S> {
    ...
    default ApiRes create(S request) {
        T savedEntity = getRepository().save(this.mapper(request, null));
        return ApiRes.success("Created successfully");
    }
     ...
     */
    default ApiRes update(S request, K id) {
        T existingEntity =
                getRepository()
        ...
        T newEntity = this.mapper(request, existingEntity);
    ...
    default ApiRes getAll(int page, int size) {
        ...
        boolean hasNextPage =
                pageData.getTotalPages() > pageable.getOffset() + pageable.getPageSize();
     ...
     */
    T mapper(S from, T to);

    /**
     * Unmaps the entity to the request object. Implementations should define how to convert
     * entity data back to request format.
     *
     ...
     */
    BaseRepository<T, K> getRepository();
