...
export const PLANT_SPECIES_CATALOG: Record<PlantSpecies, SpeciesInfo> = {
    camphor_tree: {
        ...
        description:
            'An ancient guardian tree rooted deep in the earth. Friendly forest spirits shelter in its grand canopy.',
        rarity: 'Legendary',
        seedCost: 80,
    ...
    cherry_blossom: {
        ...
        description:
            'Delicate pink petals that dance gently in the evening wind near the enchanted bathhouse bridge.',
        rarity: 'Rare',
        seedCost: 45,
    ...
    sky_bonsai: {
        ...
        description:
            'Vines intertwined with ancient stones and glowing blue levitation crystals from the floating castle.',
        rarity: 'Mythic',
        seedCost: 120,
    ...
    wildflower_meadow: {
        ...
        description:
            'A vibrant carpet of alpine poppies, lupines, and cornflowers from the secret mountain hideaway.',
        rarity: 'Common',
        seedCost: 20,
        watersNeededPerStage: 2,
        unlockedByDefault: true,
    ...
    kodama_mushrooms: {
        ...
        description:
            'Bioluminescent mushrooms and deep velvet moss where playful tree spirits rattle their heads in joy.',
        rarity: 'Rare',
        seedCost: 60,
        watersNeededPerStage: 2,
        unlockedByDefault: false,
    ...
    citrus_grove: {
        ...
        description:
            'Sun-drenched lemon and orange blossoms growing on the seaside cliffs overlooking Koriko harbor.',
        rarity: 'Common',
        seedCost: 30,
        watersNeededPerStage: 2,
        unlockedByDefault: true,
    ...
    spirit_lotus: {
        ...
        filmLore: 'Spirited Away',
        description:
            'A radiant water lily that floats above tranquil waters, surrounded by soft spirit fireflies.',
        rarity: 'Rare',
        seedCost: 70,
        watersNeededPerStage: 3,
        unlockedByDefault: false,
    ...
    golden_ginkgo: {
        ...
        description:
            'Fan-shaped golden leaves that catch the autumn sunlight as breezes carry paper airplanes into the sky.',
        rarity: 'Mythic',
        seedCost: 100,
        watersNeededPerStage: 3,
        unlockedByDefault: false,
...
export function getRandomSeedDrop(focusMinutes: number): {
    species: PlantSpecies;
    count: number;
} {
    ...
    if (focusMinutes >= 45) {
        ...
        if (roll < 0.5) return { species: 'golden_ginkgo', count: 1 };
...
export function getInitialGardenState(): GardenState {
    ...
    for (let i = 0; i < INITIAL_PLOTS_COUNT; i++) {
        ...
        const distanceFromCenter =
            Math.abs((i % 4) - 1.5) + Math.abs(Math.floor(i / 4) - 1.5);
        const unlockCost = isUnlocked
            ? 0
            : Math.round(25 + distanceFromCenter * 18);
    ...
    try {
        ...
        if (saved) {
            ...
            if (
                ...
                Array.isArray(parsed.plantedTrees)
            ) {
                return {
                    ...parsed,
                    growthPoints:
                        typeof parsed.growthPoints === 'number'
                            ? parsed.growthPoints
                            : 100,
                    seedInventory: parsed.seedInventory
                        ? { ...DEFAULT_SEED_INVENTORY, ...parsed.seedInventory }
                        : { ...DEFAULT_SEED_INVENTORY },
                    totalXpContributed:
                        typeof parsed.totalXpContributed === 'number'
                            ? parsed.totalXpContributed
                            : 0,
                };
            }
        }
...
export const STAGE_NAMES: Record<
    PlantStage,
    { label: string; progressPercent: number }
> = {
