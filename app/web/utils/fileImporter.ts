...
export async function parseFileToTable(file: File): Promise<ParsedTableData> {
    ...
    if (lowerName.endsWith('.xlsx') || lowerName.endsWith('.xls')) {
        ...
        try {
            ...
            throw new Error(
                `Failed to parse Excel file: ${err?.message || 'Unsupported format'}`
            );
        }
    } else {
    ...
    const suggestedMapping = guessColumnMapping(
        hasDetectedHeader ? headers : firstRow
    );
...
export function extractCardsFromTable(
    ...
    return rows.map((row, index) => {
        ...
        const hint =
            mapping.hintCol !== null && mapping.hintCol !== undefined
                ? (row[mapping.hintCol] || '').trim()
                : '';
        const tagsRaw =
            mapping.tagsCol !== null && mapping.tagsCol !== undefined
                ? (row[mapping.tagsCol] || '').trim()
                : '';
...
export function downloadCsvTemplate(type: 'minimal' | 'full' = 'full') {
    let csvContent = '';
    const filename = 'oopsly_flashcards_template.csv';
    ...
    if (type === 'minimal') {
        data = [
            ...
            [
                ...
                'Testing yourself to retrieve info from memory rather than passive re-reading',
            ],
            [
                'What is spaced repetition?',
                'Reviewing material at systematically increasing intervals to flatten the forgetting curve',
            ],
            [
                'What is FSRS?',
                'Free Spaced Repetition Scheduler - a modern 17-parameter memory algorithm',
            ],
        ];
    } else {
        data = [
            ...
            [
                ...
                'Think about pivot selection',
                'algorithms, sorting, computer-science',
            ],
            [
            ...
            [
                ...
                'Formula to find maximum improvement possible by improving a portion of a system',
                'Speedup equation',
