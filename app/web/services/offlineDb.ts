...
class OfflineDatabase {
        ...
        this.dbPromise = new Promise<IDBDatabase>((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('subjects')) {
                    ...
                    subjectStore.createIndex('shelfId', 'shelfId', {
                        unique: false,
                    });
                }
                ...
                if (!db.objectStoreNames.contains('cards')) {
                    ...
                    cardStore.createIndex('subjectId', 'subjectId', {
                        unique: false,
                    });
                    cardStore.createIndex('dueDate', 'dueDate', {
                        unique: false,
                    });
                    cardStore.createIndex('isDeleted', 'isDeleted', {
                        unique: false,
                    });
                }
                ...
                if (!db.objectStoreNames.contains('testSuites')) {
                    const suiteStore = db.createObjectStore('testSuites', {
                        keyPath: 'id',
                    });
                    suiteStore.createIndex('subjectId', 'subjectId', {
                        unique: false,
                    });
                }
        ...
        return new Promise<T>((resolve, reject) => {
            try {
                ...
                tx.onerror = () => {
                    reject(
                        tx.error ||
                            new Error(`Transaction error on ${storeName}`)
                    );
                };
            } catch (err) {
                reject(err);
    ...
    async getCachedShelves(): Promise<Shelf[]> {
        return this.performTransaction<Shelf[]>(
            'shelves',
            'readonly',
            (store) => store.getAll()
        );
    ...
    async getCachedSubjectById(id: string): Promise<Subject | null> {
        return this.performTransaction<Subject>(
            'subjects',
            'readonly',
            (store) => store.get(id)
        ).then((res) => res || null);
    ...
    async getCachedCardsBySubject(subjectId: string): Promise<OfflineCard[]> {
        ...
        return new Promise((resolve, reject) => {
            ...
            req.onsuccess = () => {
                const validCards = (req.result || []).filter(
                    (c: OfflineCard) => !c.isDeleted
                );
                resolve(validCards);
            };
            req.onerror = () => reject(req.error);
    ...
    async getCachedDueCardsBySubject(
        subjectId: string
    ): Promise<OfflineCard[]> {
    ...
    async getCachedCardById(id: string): Promise<OfflineCard | null> {
        return this.performTransaction<OfflineCard>(
            'cards',
            'readonly',
            (store) => store.get(id)
        ).then((res) => res || null);
    ...
    async saveOfflineCard(
        ...
        await this.performTransaction('cards', 'readwrite', (store) =>
            store.put(newCard)
        );
    ...
    async saveOfflineBatchCards(
        ...
        subjectReq.onsuccess = () => {
            ...
            if (subject) {
                subject.cardCount =
                    (subject.cardCount || 0) + createdCards.length;
                subject.dueCount =
                    (subject.dueCount || 0) + createdCards.length;
    ...
    async saveOfflineCardReview(
        ...
        card._syncStatus =
            card._syncStatus === 'pending_create'
                ? 'pending_create'
                : 'pending_update';
        ...
        let totalXp = 0;
        try {
            const meta = (await this.getMetadata<{ xp: number }>(
                'user_profile_xp'
            )) || { xp: 0 };
    ...
    async saveOfflineCardUpdate(
        cardId: string,
        data: Partial<Card>
    ): Promise<OfflineCard | null> {
    ...
    async replaceTempCardId(tempId: string, realCard: Card): Promise<void> {
        ...
        queueReq.onsuccess = () => {
            ...
            actions.forEach((act) => {
                ...
                if (
                    act.type === 'GRADE_CARD' &&
                    act.payload.cardId === tempId
                ) {
                    act.payload.cardId = realCard.id;
                    changed = true;
                } else if (
                    act.type === 'UPDATE_CARD' &&
                    act.payload.cardId === tempId
                ) {
                    act.payload.cardId = realCard.id;
                    changed = true;
                } else if (
                    act.type === 'DELETE_CARD' &&
                    act.payload.cardId === tempId
                ) {
                    act.payload.cardId = realCard.id;
                    changed = true;
    ...
    async getMetadata<T = any>(key: string): Promise<T | null> {
        const item = await this.performTransaction<{ key: string; value: T }>(
            'metadata',
            'readonly',
            (store) => store.get(key)
        );
