...
//                        .tags(request.tags())
    ...
    public ApiRes getAllByShelve(UUID shelfId, int page, int size) {
        ...
        int queryPage = page - 1; // Adjust for zero-based indexing
        if (queryPage < 0) {
            throw new ValidationException("Page number must be greater than 0");
        }
        Shelf shelf = getShelfForCurrentUser(shelfId);
        Pageable pageable = PageRequest.of(queryPage, size);
        Page<Subject> pageData = subjectRepository.findAllByShelve(shelf, pageable);
    ...
    private SubjectRes toSubjectRes(Subject entity) {
    ...
    public ApiRes cloneDeck(UUID subjectId) {
        ...
        Shelf targetShelf = shelves.get(0);
        ...
        Subject cloned =
                Subject.builder()
                        ...
                        .shelf(targetShelf)
                        .build();
        Subject savedClone = subjectRepository.save(cloned);
    ...
    private Shelf getShelfForCurrentUser(UUID shelfId) {
        User currentUser = userService.getCurrentUser();
        log.debug(
