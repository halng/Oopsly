...
public class TestSuiteController {
    ...
    @Operation(
            ...
            description =
                    "Grades the answers of a finished test run, awards XP, updates the highest"
                            + " score of the preset and returns a per-question breakdown with"
                            + " explanations")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Test submitted and graded successfully",
                        content = @Content(schema = @Schema(implementation = ApiRes.class))),
                @ApiResponse(responseCode = "400", description = "Invalid submission payload"),
                @ApiResponse(responseCode = "404", description = "Test suite or shelve not found"),
                @ApiResponse(responseCode = "503", description = "Test suite service degraded")
            })
    @PostMapping("/{id}/submit")
    public ApiRes submitRun(
            @Parameter(
                            description = "Shelve ID",
                            required = true,
                            example = "123e4567-e89b-12d3-a456-426614174000")
            ...
            @Parameter(
                    ...
                    @PathVariable
                    UUID id,
            @Parameter(description = "Answers and elapsed time", required = true)
                    @Valid @RequestBody
                    TestSubmissionReq requestBody) {
        return this.service.submit(shelveId, id, requestBody);
    }
}
